...
로그·텍스트의 반복 분류를 로컬에서 실행 중인 Kev에 우선 맡깁니다. 주 모델의
직접 판단이나 다른 모델 호출이 더 저렴하거나 빨라 보여도 Kev를 먼저 호출합니다.
...
**실행 중인 로컬 Kev를 우선 호출합니다.** 위 의미 판단은 짧은 발췌라도 코딩
에이전트의 주 모델이 직접 답하거나 다른 모델에 요청하기 전에 Kev에 먼저 맡깁니다.
예상 토큰 비용, 직접 읽기의 부담, 외부 추론 속도를 이유로 Kev를 건너뛰지 않습니다.
요청을 짧게 유지하고 독립적인 질문을 묶어 호출 부담을 줄입니다.

자동 선택은 모든 작업에서의 호출을 보장하지 않습니다. 다음처럼 의미 판단이
필요하지 않은 경우에는 추론을 생략합니다.

- 정확한 문자열 검색, 파일 존재 확인 등 일반 코드로 처리할 수 있는 작업
- `rank`에서 평가 가능한 후보 수가 `top-k` 이하인 경우: 기본값은 3개

서버 연결 실패, 시간 초과, 유효하지 않거나 불확실한 응답이면 이유를 알리고
코딩 에이전트의 일반 조사로 돌아갑니다. 반복 재시도하거나 별도의 외부 분류 모델로
전환하지 않습니다. 비용만으로는 대체 처리로 넘어가지 않습니다.
...
| 서버는 켜져 있지만 호출이 없음 | 설치본의 자동 선택 설정, 적용되는 `AGENTS.md`/`CLAUDE.md`, 파일 선별·반복 분류 조건을 확인합니다. 비용을 이유로 생략하도록 안내하는 이전 스킬과 snippet은 갱신하세요. 진단 시 명시 호출로 연결 문제와 구분할 수 있습니다. |
